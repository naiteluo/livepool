...
var tbProtoBuf = require('../plugins/tbprotobuf');
...
exports.response = function(id, req, res, body) {
    ...
    // do tieba protobuf decode
    if (res.getHeader('content-type') === 'application/protobuf') {
        var urlData = url.parse(req.url);
        var params = qs.parse(url.parse(req.url).query);

        body = tbProtoBuf.decode('res', params.cmd, body);
        body = util.escapedStringify(body);
    } else {
...
/**
 * process form-data from protobuf
 * @param  {[type]} id  [description]
 * @param  {[type]} req [description]
 * @param  {[type]} res [description]
 * @return {[type]}     [description]
 */
exports.reqFormData = function (id, req, res) {
    socket = liveapp.socket;
    if (socket && checkUrl(req.url)) {
        // Parse an incoming multipart/form-data request.
        if ((req.headers['content-type'] || '').indexOf('multipart/form-data') >= 0
            && req.getHeader('x_bd_data_type') === 'protobuf') {
            try {
                ...
                form.parse(req, function(err, fields, files) {
                    if (files.data && files.data.length > 0) {
                        var buf = fs.readFileSync(files.data[0].path);
                        var reqData = tbProtoBuf.decode('req', params.cmd, buf);
                    }
                    sid = socket.id + '_' + id;
                    queue.push({
                        type: 'res',
                        rows: [{
                            id: sid,
                            // data in queue will be eacape once, unescape before to keep data right
                            reqBody: qs.unescape(JSON.stringify(reqData))
                        }]
                    });
                });
            } catch (e) {
