var inspectorHandler = {
    ...
    inspect: function(record) {
        ...
        // req view
        var reqViewContainer = $cmp.inspactors_req_view_container;
        if (record.data.reqHeaders.x_bd_data_type === 'protobuf') {
            // text view
            REQ_VIEWER._type = 'ProtoBufReq';
            Ext.getCmp('inspactors_req_view_Text').toggle(true);
            REQ_VIEWER.showTextViewer(reqViewContainer, record);
        } else {
            REQ_VIEWER._type = 'ProtoBufReq';
            Ext.getCmp('inspactors_req_view_Text').toggle(true);
            REQ_VIEWER.showTextViewer(reqViewContainer, record);
...
var inspactorsReqViewToolbarToggle = function(btn, pressed) {
    if (pressed && $cmp.editRecord) {
        var el = $cmp.inspactors_req_view_container;
        var record = $cmp.editRecord;
        REQ_VIEWER._type = 'ProtoBufReq';
        switch (btn.text) {
            case 'Image':
                REQ_VIEWER.showImageViewer(el, record);
                break;
            case 'JSON':
                REQ_VIEWER.showJsonViewer(el, record);
                break;
            case 'Text':
                REQ_VIEWER.showTextViewer(el, record);
                break;
            case 'Other':
                REQ_VIEWER.showOtherViewer(el, record);
                break;
            case 'Raw':
                REQ_VIEWER.showRawViewer(el, record);
                break;
            default:
        }
    }
...
var REQ_VIEWER = {
    ...
    _typeChanged: function() {
        return;
        var inspactors_req_view_tool_unicode = Ext.getCmp('inspactors_req_view_tool_unicode');
        var inspactors_req_view_tool_beautify = Ext.getCmp('inspactors_req_view_tool_beautify');
        var inspactors_req_view_tool_save = Ext.getCmp('inspactors_req_view_tool_save');
        var inspactors_req_view_tool_addrule = Ext.getCmp('inspactors_req_view_tool_addrule');
        var inspactors_req_view_tool_download = Ext.getCmp('inspactors_req_view_tool_download');
        switch (REQ_VIEWER._type) {
            case 'Image':
                inspactors_req_view_tool_unicode.setDisabled(true);
                inspactors_req_view_tool_beautify.setDisabled(true);
                inspactors_req_view_tool_save.setDisabled(true);
                inspactors_req_view_tool_addrule.setDisabled(false);
                inspactors_req_view_tool_download.setDisabled(false);
                break;
            case 'JSON':
                inspactors_req_view_tool_unicode.setDisabled(true);
                inspactors_req_view_tool_beautify.setDisabled(true);
                inspactors_req_view_tool_save.setDisabled(false);
                inspactors_req_view_tool_addrule.setDisabled(false);
                inspactors_req_view_tool_download.setDisabled(false);
                break;
            case 'Text':
                inspactors_req_view_tool_unicode.setDisabled(false);
                inspactors_req_view_tool_beautify.setDisabled(false);
                inspactors_req_view_tool_save.setDisabled(false);
                inspactors_req_view_tool_addrule.setDisabled(false);
                inspactors_req_view_tool_download.setDisabled(false);
                break;
    ...
    showJsonViewer: function(el, record) {
        // features: paste text -> json view
        var textValue = '';
        if (REQ_VIEWER._type == 'Text') {
            textValue = REQ_VIEWER._el.getValue();
        }

        if (REQ_VIEWER._type == 'ProtoBufReq') {
            textValue = record.data.reqBody;
        }
        ...
        var panel = new Ext.Panel({
            ...
            flex: 1,
            margin: '0 0 4 0',
            html: '<div id="jsonPanelReq" style="height:100%;"></div>'
        });
        el.add(panel);
        var jsonPanel = document.getElementById('jsonPanelReq');
        ...
        function setJsonView(value) {
            ...
            $cmp.jsoneditorreq = new jsoneditor.JSONEditor(jsonPanel, options, json);
        }
        if (textValue) {
            setJsonView(textValue);
        } else {
        ...
        REQ_VIEWER._type = 'JSON';
        REQ_VIEWER._el = jsonPanel;
        REQ_VIEWER._typeChanged();
    },
    showOtherViewer: function(el, record) {
        el.removeAll();
        var panel = new Ext.Panel({
        ...
        el.add(panel);
        REQ_VIEWER._type = 'Other';
        REQ_VIEWER._el = panel;
        REQ_VIEWER._typeChanged();
    },
    showTextViewer: function(el, record) {
        el.removeAll();
        ...
        el.add(textArea);
        if (REQ_VIEWER._type == 'ProtoBufReq') {
            textArea.setValue(record.data.reqBody);
        } else {
            Ext.Ajax.request({
                url: '/res/get',
                params: {
        ...
        REQ_VIEWER._type = 'Text';
        REQ_VIEWER._el = textArea;
        REQ_VIEWER._typeChanged();
    },
    showRawViewer: function(el, record) {
        el.removeAll();
...
var TAB_INSPECTORS_REQ = {
    ...
    items: [{
        ...
        id: 'inspactors_req_view',
        title: 'ProtoBufReqest',
        layout: {
            type: 'vbox',
            align: 'stretch'
        },
        items: [{
            id: 'inspactors_req_view_container',
            xtype: 'container',
            flex: 1,
            layout: {
                type: 'vbox',
            ...
            border: 1,
            items: [{
                    id: 'inspactors_req_view_JSON',
                    xtype: 'button',
                    text: 'JSON',
                    iconCls: 'icomoon-list3',
                    enableToggle: true,
                    toggleGroup: 'reqViewType',
                    toggleHandler: inspactorsReqViewToolbarToggle
                }, {
                    id: 'inspactors_req_view_Text',
                    xtype: 'button',
                    text: 'Text',
                    enableToggle: true,
                ...
                '->', {
                    id: 'inspactors_req_view_tool_save',
                    xtype: 'button',
                    tooltip: 'Save',
                    text: '',
                    iconCls: 'icomoon-disk',
